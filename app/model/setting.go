...
const stmtGetSetting = `SELECT * FROM settings WHERE key_word = ?`
const stmtSaveSelect = `SELECT id FROM settings WHERE key_word = ?`
...
type Setting struct {
	...
	KeyWord   string     `meddler:"key_word"`
...
func (setting *Setting) GetSetting() error {
	err := meddler.QueryRow(db, setting, stmtGetSetting, setting.KeyWord)
	return err
}
...
func (setting *Setting) Save() error {
	var id int
	row := db.QueryRow(stmtSaveSelect, setting.KeyWord)
